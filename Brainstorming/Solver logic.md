...
#### Exemple 2.1
##### Fenêtres
On a les 3 fenêtres suivantes, dans l'ordre :
1. A =  auto
2. B = auto
3. C = auto
...
#### Exemple 3.1
##### Fenêtres
On a les 2 fenêtres suivantes, dans l'ordre :
1. A =  auto
2. B = auto
...
#### Exemple 3.5
##### Fenêtres
On a les 6 fenêtres suivantes, dans l'ordre :
1. A =  auto
2. B = auto
3. C = auto
4. D = auto
...
#### Exemple 3.6
##### Fenêtres
On a les 7 fenêtres suivantes, dans l'ordre :
1. A =  auto
2. B = auto
3. C = auto
4. D = auto
...
#### Exemple 3.7
##### Fenêtres
On a les 8 fenêtres suivantes, dans l'ordre :
1. A =  auto
2. B = auto
3. C = auto
4. D = auto
...
### Exemple 4
#### Configuration
On reprend la configuration de l'[[#Exemple 2]] :
```
        allowed_dimensions = {
			{ 0.66, 1.0 },
			{ 0.5, 1.0 },
...
Ces exemples servent à challenger le comportement du solver avec des dimensions forcées dans une configuration où les dimensions en tiers existent, mais où les colonnes de largeur `0.33` ne peuvent contenir que 1 ou 2 fenêtres.
...
Ce cas vérifie que le solver garde la dimension forcée au milieu de l'ordre, réduit les groupes auto autour d'elle pour limiter le scroll, et ne force pas toutes les fenêtres auto à la largeur `0.5`.
...
Ce cas vérifie qu'une grande fenêtre forcée en début d'ordre peut provoquer du scroll, mais que les fenêtres auto restantes utilisent la largeur minimale permettant de limiter l'extent.
...
Ce cas vérifie qu'une fenêtre forcée en pleine hauteur avec largeur `0.33` peut rester dans un layout sans scroll, et que les fenêtres auto voisines ne doivent pas être agrandies si cela introduit du scroll.
...
Ce cas vérifie qu'un groupe auto impair avant une grande fenêtre forcée est résolu en utilisant une colonne partagée puis une colonne pleine hauteur, plutôt qu'en augmentant inutilement la largeur de toutes les fenêtres auto.
...
Ce cas vérifie que deux fenêtres forcées partielles et consécutives peuvent partager une même colonne si leurs dimensions remplissent l'axe opposé du scroll.

### Exemple 5
#### Configuration
On reprend la configuration de l'[[#Exemple 3]] :
```
        allowed_dimensions = {
			{ 1.0, 1.0 },
			{ 0.66, 1.0 },
...
Ces exemples servent à challenger le comportement du solver avec une configuration plus riche, où plusieurs remplissages partiels sont possibles sur une même largeur.
...
Ce cas vérifie qu'une fenêtre forcée très large au milieu conserve sa position logique, même si cela force les fenêtres auto autour d'elle à utiliser une colonne partielle en largeur `0.33`.
...
Ce cas vérifie qu'une dimension forcée large mais partielle peut être complétée par une fenêtre auto de même largeur dans la même colonne.
...
Ce cas vérifie qu'une fenêtre forcée en plein écran peut coexister avec un layout auto dense après elle, sans que le solver tente de réduire la fenêtre forcée.
...
Ce cas vérifie que le solver peut réduire les groupes auto autour d'une fenêtre forcée pleine hauteur pour minimiser l'extent total.
...
Ce cas vérifie que plusieurs dimensions forcées partielles peuvent former une colonne presque pleine en tiers, et que le solver traite `0.66 + 0.33 = 0.99` comme un remplissage complet.
